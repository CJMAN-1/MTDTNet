...
class Trainer:
    ...
    def train_dis(self, imgs, labels):  # Train Discriminators(D)
        ...
        with torch.no_grad():
            for dset in self.datasets:
                ...
                contents[dset], styles[dset] = self.nets['CSE'](T_feature[dset])

            for convert in self.converts:
                source, target = convert.split('2')
                converted_imgs[convert] = self.nets['G']((contents[source], styles[target]))
    ...
    def train_task(self, imgs, labels):  # Train Task Networks (T)
        ...
        #     self.nets['T'](converted_T_feature[convert], lbl=labels[self.source], weight=class_weight, from_layer1=True)
        #     loss_task += self.nets['T'].loss_seg
        loss_task.backward()
        self.optims['T'].step()
    ...
    def train_esg(self, imgs, labels):
        ...
        D_outputs_fake = dict()
        contents, styles = dict(), dict()
        converted_content, converted_style = dict(), dict()
        for dset in self.datasets:
            _, T_feature[dset] = self.nets['T'](imgs[dset])  # B x 256 x 129 x 257
            T_feature[dset] = F.interpolate(T_feature[dset], size=(self.h//4, self.w//4), mode='bilinear')  # B x 256 x 128 x 256
            direct_recon[dset] = self.nets['G'](T_feature[dset], True)
            Recon_loss += self.loss_fns.recon_single(self.down(imgs[dset]), direct_recon[dset])
            contents[dset], styles[dset] = self.nets['CSE'](T_feature[dset])
            indirect_recon[dset] = self.nets['G']((contents[dset], styles[dset]))
            Recon_loss += self.loss_fns.recon_single(self.down(imgs[dset]), indirect_recon[dset])
        ...
        for convert in self.converts:
            ...
            converted_content[convert], converted_style[convert] = self.nets['CSE'](converted_T_feature[convert])
        ...
        for convert in self.converts:
            ...
            Consis_loss += self.loss_fns.recon_single(styles[target], converted_style[convert])
