...
class Trainer:
    ...
    def set_networks(self):
        ...
        self.nets['M'] = Adaptive_Style_Memory_Bank(self.targets)
        # initialization
    ...
    def train_dis(self, imgs, labels):  # Train Discriminators(D)
        ...
        for target in self.targets:
            ...
            D_outputs_real[target] = self.nets['D'][target](slice_patches(imgs[target]))
        for convert in self.converts:
    ...
    def train_task(self, imgs, labels):  # Train Task Networks (T)
        ...
        _, last_feature = self.nets['T'](imgs[self.source], lbl=labels[self.source], weight=class_weight)
    ...
    def train_esg(self, imgs, labels):
        ...
        for convert in self.converts:
            ...
            converted_contents[convert], converted_styles[convert] = self.nets['CSE'](cycle_converted_features[convert])
    ...
    def print_loss(self):
        ...
        self.logger.info(
            '[%d/%d] D: %.2f| G: %.2f| R: %.2f| C: %.2f| %s %s'
            % (self.step, self.opt.iter,
               self.losses['D'], self.losses['G'], self.losses['R'], self.losses['C'], best_mious, self.opt.ex))
    ...
    def train(self):
        ...
        for i in range(self.opt.iter):
            ...
            # evaluation
            if self.step % self.opt.eval_freq == 0 and self.step > 700:
                for target in self.targets:
                    self.eval(target)
            self.print_loss()
