...
class Trainer:
    def __init__(self, opt):
        ...
        if opt.super_class:
            self.n_class = 7
            self.current_best_T = './checkpoint/M_seg_super_pretrain/80.77/netT'
            for target in self.targets:
                if target == 'C':
                    self.best_miou[target] = 80.0
                    self.min_miou[target] = 60.0
                elif target == 'I':
                    self.best_miou[target] = 75.0
                    self.min_miou[target] = 60.0
                elif target == 'M':
    ...
    def set_networks(self):
        ...
        for key in ['E', 'SE', 'LE', 'DT', 'G']:
            ...
            # self.nets[key].load_state_dict(torch.load(folder1 + 'net%s.pth' % (key)))
    ...
    def train(self):
        ...
        for i in range(self.opt.iter):
            ...
            while not self.reload:
                ...
                # evaluation
                if self.step % self.opt.eval_freq == 0 and self.step > 0:
                    self.skip = False
                    for target in self.targets:
                        if not self.skip:
                            self.eval(target)
