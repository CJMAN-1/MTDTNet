...
class Losses:
    def __init__(self, opt):
        ...
        self.loss_fns['IF'] = Inverse_Focal_CELoss(self.opt.n_class)
        if self.opt.cuda:
            for fn in self.loss_fns.values():
                fn.cuda()
        self.targets = opt.datasets[1:]
        self.n_targets = len(opt.datasets) - 1
        self.alpha_recon = 10
    ...
    def inverse_focal_loss(self, f, domain, gamma=2, lambda_f = 1):
        return self.loss_fns['IF'](f, domain, gamma, lambda_f)
...
class Inverse_Focal_CELoss(nn.Module):
    def __init__(self, n_class=19):
        super().__init__()
        self.n_class = n_class

    def forward(self, predict, target, gamma=1.5, lambda_f = 1):
        ...
        # Todo: make target that shapes like predict
        # Ignore label is considered?
        target_cls = torch.zeros_like(predict).float()
        for cls in range(self.n_class):
            # print("cls :", cls)
            target_cls[:,cls] = (target == cls) * target
        
        # print(torch.sum(target_cls) == 0)
        pred_ = F.softmax(predict, dim=1)
        log_pred = F.log_softmax(predict, dim=1)
        loss = - torch.mean(torch.pow(pred_, gamma) * target_cls * log_pred )  # p**gamma * tlogp
