...
class Trainer:
    ...
    def set_networks(self):
        if not self.super_class:
            ...
            self.nets['T'] = Deeplab(num_classes=self.n_class, restore_from='pretrained_model/deeplab_city_sc_63.20')
    ...
    def eval(self):
        ...
        with torch.no_grad():
            for batch_idx, (imgs, labels) in enumerate(self.test_loader[self.target]):
                if self.opt.cuda:
                ...
                progress_bar(batch_idx, len(self.test_loader[self.target]), 'mIoU: %.3f' % miou)
            # Save checkpoint.
            self.logger.info('======================================================')
