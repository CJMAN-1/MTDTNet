...
def make_list():
    ...
    # for mode in ['train', 'val']:
        ...
        #     image_root = os.path.join('../IDD', 'leftImg8bit', mode)
        #     label_root = os.path.join('../IDD', 'gtFine', mode)
        ...
        #             image_file = os.path.join('/data/datasets/IDD/leftImg8bit',mode,folder, image)
        #             label_file = os.path.join('/data/datasets/IDD/gtFine',mode,folder, label)
        #             f1.write(image_file + '\n')
        #             f2.write(label_file + '\n')
        #     f1.close()
        #     f2.close()
    ...
    for mode in ['training', 'validation']:
        ...
        if not os.path.exists(image_text_file) or not os.path.exists(label_text_file):
            ...
            label_root = os.path.join('/data', 'datasets', 'mapillary', mode, 'v1.2/labels')
            # print(image_root)
            # file_list = os.listdir(image_root)
            f1 = open(image_text_file, mode='wt')
            f2 = open(label_text_file, mode='wt')
            ...
            for image in image_list:
                # image_name_split = image.split('_')
                label = image.split('.')[0] + '.png'
                image_file = os.path.join(image_root, image)
                label_file = os.path.join(label_root, label)
                print(image_file)
                print(label_file)
                f1.write(image_file + '\n')
                f2.write(label_file + '\n')
