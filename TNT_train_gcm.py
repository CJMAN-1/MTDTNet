...
class Trainer:
    def __init__(self, opt):
        ...
        if opt.super_class:
            self.n_class = 7
            self.current_best_T = 'SOTA_pretrained/G2CM_super/_73.18_72.35/netT'
            for target in self.targets:
                if target == 'C':
                    self.best_miou[target] = 73.0
                    self.min_miou[target] = 59.0
                elif target == 'I':
                    self.best_miou[target] = 59.8
                    self.min_miou[target] = 58.0
                elif target == 'M':
                    self.best_miou[target] = 72.0
                    self.min_miou[target] = 59.0
        else:
            self.n_class = 19
            # self.current_best_T = '../wh_DRANet/checkpoint/45.63_43.16/_44.47_44.75/netT'
            self.current_best_T = './checkpoint/GCM_Super_PixelDA/_45.58_48.73/netT'
            # self.current_best_T = 'pretrained_model/deeplab_gta5_36.94'
            for target in self.targets:
                if target == 'C':
                    self.best_miou[target] = 45.10
    ...
    def set_networks(self):
        ...
        folder1 =  'SOTA_pretrained/G2CM_super/_73.18_72.35/'
