...
netT = Deeplab(num_classes=19, init_weights='pretrained/DeepLab_init.pth')
...
def test(epoch, test_loader, best_acc, save=True, dataset="target", is_plot=False):
    ...
    with torch.no_grad():
        ...
        if dataset == 'source' and miou > best_acc:
            torch.save(netT.state_dict(), './pretrained_model/drn26_gta5.pth')
            return miou
        else:
            return best_acc
...
for epoch in range(n_iter):
    ...
    best_miou = test(-1, source_train_loader, best_miou, save=False, dataset="source")