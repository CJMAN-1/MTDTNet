...
class Trainer:
    ...
    def set_networks(self):
        ...
        self.nets['D'] = dict()
        for target in self.targets:
            self.nets['D'][target] = Perceptual_Discriminator()
            # self.nets['D'][target] = PatchGAN_Discriminator()
        ...
        for net in self.nets.keys():
            if net == 'D':
                for target in self.targets:
                    init_params(self.nets['D'][target])
            else:
    ...
    def set_optimizers(self):
        ...
        self.optims['D'] = dict()
        for target in self.targets:
            self.optims['D'][target] = optim.Adam(self.nets['D'][target].parameters(), lr=self.opt.lr_dra,
                                                    betas=(self.opt.beta1, 0.999),
                                                    weight_decay=self.opt.weight_decay)
    ...
    def set_zero_grad(self):
        for net in self.nets.keys():
            if net == 'D':
                for target in self.targets:
                    self.nets[net][target].zero_grad()
            else:
    ...
    def train_dis(self, imgs, labels):  # Train Discriminators(D)
        ...
        with torch.no_grad():
            for dset in self.datasets:
                ...
                contents[dset], _ = self.nets['CSE'](features[dset])
                styles[dset] = features[dset] - w[dset]*contents[dset]
            ...
            for convert in self.converts:
                ...
                # converted_features[convert] = w[convert]*contents[source] + styles[target]
                converted_features[convert] = adaptive_instance_normalization(features[source], features[target])
        ...
        for target in self.targets:
            ...
            # D_outputs_real[dset] = self.nets['D'][target](slice_patches(imgs[dset]))
        for convert in self.converts:
            D_outputs_fake[convert] = self.nets['D'][convert[-1]](converted_imgs[convert])
            # D_outputs_fake[convert] = self.nets['D'][convert[-1]](slice_patches(converted_imgs[convert]))
            
        loss_dis = self.loss_fns.dis_patch(D_outputs_real, D_outputs_fake)
        loss_dis.backward()
        for target in self.targets:
            self.optims['D'][target].step()
    ...
    def train_esg(self, imgs, labels):
        ...
        perceptual = dict()
        styles = dict()
        _, perceptual[self.source] = self.nets['D'][self.targets[0]](imgs[self.source])
        for dset in self.datasets:
            features[dset] = self.nets['E'](imgs[dset])
            contents[dset], _= self.nets['CSE'](features[dset])
            w[dset] = self.nets['W'](features[dset], features[dset], dset, dset)
            styles[dset] = features[dset] - w[dset]*contents[dset]
            # indirect_features[dset] = adaptive_instance_normalization(contents[dset], features[dset])
            # indirect_features[dset] = self.nets['CSG'](contents[dset], styles[dset])
        ...
        for convert in self.converts:
            ...
            # converted_features[convert] = w[convert]*contents[source] + styles[target]
            converted_imgs[convert] = self.nets['G'](converted_features[convert])
            D_outputs_fake[convert], perceptual[convert] = self.nets['D'][target](converted_imgs[convert])
        ...
        G_loss = self.loss_fns.gen_patch(D_outputs_fake)
        ...
        for convert in self.converts:
            ...
            # Consis_loss += self.loss_fns.recon_single(contents[source], cycle_w[convert]*converted_contents[convert])
            Consis_loss += F.mse_loss(perceptual[source], perceptual[convert])
    ...
    def tensor_board_log(self, imgs, labels):
        ...
        with torch.no_grad():
            ...
            for dset in self.datasets:
                ...
                # indirect_features[dset] = adaptive_instance_normalization(w[dset]*contents[dset], features[dset])
                direct_recon[dset] = self.nets['G'](features[dset])
                # indirect_recon[dset] = self.nets['G'](indirect_features[dset])
