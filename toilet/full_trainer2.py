...
class Trainer:
    ...
    def set_networks(self):
        ...
        for dset in self.datasets:
            self.nets['D'][dset] = FPSE_Discriminator(self.n_class)
        # initialization
    ...
    def set_optimizers(self):
        self.optims['C'] = optim.Adam(self.nets['C'].parameters(), lr=self.opt.lr_dra,
    ...
    def train_cls(self, src_labels, tgt_labels):
        self.set_zero_grad()
        src_score = self.nets['C'](src_labels)   
        tgt_score = self.nets['C'](tgt_labels)   
        ones = torch.ones_like(tgt_score)

        src_loss = src_score.mean()
        tgt_loss = (1. - tgt_score).mean()
        src_loss.backward()
        tgt_loss.backward()
        self.optims['C'].step

        return float(src_loss), float(tgt_loss) 
    ...
    def train_dis(self, imgs, labels):  # Train Discriminators(D)
        ...
        for dset in self.opt.datasets:
            ...
            D_outputs_real[dset] = self.nets['D'][dset](imgs[dset], labels[dset])
            # D_outputs_real[dset] = self.nets['D'][dset](slice_patches(imgs[dset]))
        with torch.no_grad():
            contents, styles = self.nets['S'](features, labels, converts = self.converts)
        ...
        D_outputs_fake[self.s2t] = self.nets['D'][self.target](converted_imgs[self.s2t], labels[self.source])
        D_outputs_fake[self.t2s] = self.nets['D'][self.source](converted_imgs[self.t2s], labels[self.target])
    ...
    def train_esg(self, imgs, labels):
        ...
        contents, styles = self.nets['S'](features, labels, converts=self.converts)
        for dset in self.opt.datasets:
            recon_imgs_2[dset] = self.nets['G'](contents[dset], styles[dset])
        ...
        converted_contents, converted_styles = self.nets['S'](converted_features, labels)
        ...
        Recon_loss = self.loss_fns.recon(imgs, recon_imgs) + self.loss_fns.recon(imgs, recon_imgs_2)
    ...
    # count the pixel per class for mini-batch (2 images), 
    # return the use/non-use for input data with threshold-based rule
    # INPUT : input data
    # def pixelcount(self):
    #     self.class_num = 19
    ...
    def train(self):
        ...
        for i in range(self.opt.iter):
            ...
            # print(labels['G'].size())
            src_score, tgt_score = self.train_cls(labels['G'].float(), labels['C'].float())
            print("SOURCE SCORE =", float(src_score))
            print("TARGET SCORE =", float(tgt_score))
            if src_score > 0.5:
                for j in range(5):
